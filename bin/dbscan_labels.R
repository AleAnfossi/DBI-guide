dbscan_labels<-function(data,eps,minPts,case)
{
  ...
  switch(case,
         "Neuroblastoma" = {
           dbscan_smal <- dbscan(data, eps=eps-0.2, minPts=minPts+1, borderPoints=TRUE)$cluster
         ...
         "Heart_failure" = {
           dbscan_smal <- dbscan(data, eps=eps-10, minPts=minPts+2, borderPoints=TRUE)$cluster
           dbscan_med  <- dbscan(data, eps=eps, minPts=minPts, borderPoints=TRUE)$cluster
           dbscan_big  <- dbscan(data, eps=eps+10, minPts=minPts-2, borderPoints=TRUE)$cluster
         },
         "Diabetes" = {
           dbscan_smal <- dbscan(data, eps=eps-3, minPts=minPts+1, borderPoints=TRUE)$cluster
           dbscan_med  <- dbscan(data, eps=eps, minPts=minPts, borderPoints=TRUE)$cluster
           dbscan_big  <- dbscan(data, eps=eps+3, minPts=minPts-1, borderPoints=TRUE)$cluster
         },
